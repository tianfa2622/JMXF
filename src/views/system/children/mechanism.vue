<template>
  <div class="system_content">
    ...
    <el-dialog :title="deltitle" :visible.sync="delShow" center width="30%">
      <p v-if="deltitle === '删除'">
        确认删除该组织机构吗？<br />
        删除前请检查该组织机构下没有用户数据，否则无法删除！
      </p>
      <p v-else>确认要禁用该组织机构吗？</p>
      <template #footer>
        <span class="dialog-footer">
  ...
  data() {
    return {
      ...
      searchSettings: [
        ...
        { placeholder: '请输入机构代码或机构名称', type: 'input', name: 'input' }
      ],
      // 弹出框
      dialogSettings: [
        { label: '机构代码', placeholder: '请输入机构代码', type: 'input', name: 'input' },
        { label: '机构名称', placeholder: '请输入机构名称', type: 'input', name: 'input' }
      ],
      // 搜索组件按钮属性
      searchBtn: [
        { name: '查询', type: 'search' },
